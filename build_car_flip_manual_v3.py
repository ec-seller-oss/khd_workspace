...
def footer(s):
    ...
    txt(s,Inches(0.5),Inches(7.1),Inches(10.0),Inches(0.3),'KHD 車両転売事業マニュアル v3.1（公用車入札・目利き完全版｜2026-07-19更新）',size=8,color=GRAY)
...
txt(s,Inches(0.92),Inches(6.45),Inches(11),Inches(0.4),'KIKUCHIホールディングス株式会社 ｜ 初版2026-07-15／v3.1更新2026-07-19 ｜ H037テンプレ準拠',size=10,color=GRAY)


# ============ 1b. 更新履歴（何をやってきたかを思い出すページ） ============
s=prs.slides.add_slide(BLANK); bg(s)
header(s,'更新履歴','このマニュアルの育ち方（日付で振り返る）','データが増えるたびにここへ1行追記していく')
logs=[
 ('7/14','案件精査・WBS','岩手中部水道企業団148号の正体特定。資格(花巻支店)クリア確認、様式下書き、相見積りWBS、カレンダー10件登録'),
 ('7/15','写真・グレード確定','実車写真9枚からVXバッジ・ODO28万km一致・5MTを現物確認。旧車王 実査定100〜200万円を取得。目利き完全版(v2)作成'),
 ('7/17','入札ウォッチャー','15サイト巡回→スプシ記帳→LINE通知のGAS稼働。案件パイプライン・販売先マスタを自動生成'),
 ('7/18','相場マスタ','仕入れ×出口の掛け目一覧(車0.5/物0.6/不動産0.5)。岩手県の公用車入札を検知'),
 ('7/19','会議・決定木・落札事例','ガリバー友人会議(AIS鑑定/マフラー穴/記録簿/整備総額)を目利きに反映。入札額の決定木確定。官公庁ランクル落札事例を発掘(→p23)'),
]
y=Inches(1.9)
for d,t,desc in logs:
    box(s,Inches(0.55),y,Inches(11.9),Inches(0.88),fill=BEIGE)
    box(s,Inches(0.55),y,Inches(1.15),Inches(0.88),fill=RED)
    txt(s,Inches(0.55),y,Inches(1.15),Inches(0.88),d,size=13,color=WHITE,bold=True,align=PP_ALIGN.CENTER,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(1.9),add(y,Inches(0.08)),Inches(2.3),Inches(0.7),t,size=12.5,color=DRED,bold=True,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(4.3),add(y,Inches(0.08)),Inches(8.0),Inches(0.75),desc,size=10.5,color=INK,sp=1.15,anchor=MSO_ANCHOR.MIDDLE)
    y=add(y,Inches(0.99))
footer(s)
...
# ============ 20b. 官公庁ランクル落札事例 ============
s=prs.slides.add_slide(BLANK); bg(s)
header(s,'データ ｜ 2026-07-19追加','官公庁オークションのランクル落札事例：予定価格の9〜20倍','「公用車平均15.7万」はランクルには当てはまらない')
rows=[('ランクル70バン(KC-HZJ77V)・兵庫県競馬組合','1998年式・4.9万km・傷あり','予定20万円','321.3万円(入札119件)','約16倍'),
      ('ランクルプラド消防指揮車・香美町','1995年式・2.8万km','—','311.1万円','—'),
      ('ランクル消防車・鯖江市','2003年式・0.9万km','—','246万円','20倍超'),
      ('ランクル消防ポンプ車・村田町','1977年式・0.7万km','—','88.9万円','約9倍')]
y=Inches(1.85)
box(s,Inches(0.55),y,Inches(11.9),Inches(0.5),fill=RED)
for ci,(cw,label) in enumerate([(4.3,'事例'),(2.6,'年式/走行'),(1.5,'予定価格'),(2.2,'落札価格'),(1.3,'倍率')]):
    x=sum([4.3,2.6,1.5,2.2,1.3][:ci]); txt(s,add(Inches(0.7),Inches(x)),y,Inches(cw),Inches(0.5),label,size=11.5,color=WHITE,bold=True,anchor=MSO_ANCHOR.MIDDLE)
y=add(y,Inches(0.5))
for r_i,row in enumerate(rows):
    box(s,Inches(0.55),y,Inches(11.9),Inches(0.62),fill=BEIGE if r_i%2==0 else WHITE)
    for ci,val in enumerate(row):
        x=sum([4.3,2.6,1.5,2.2,1.3][:ci]); cw=[4.3,2.6,1.5,2.2,1.3][ci]
        txt(s,add(Inches(0.7),Inches(x)),add(y,Inches(0.03)),Inches(cw),Inches(0.58),val,size=10.5,color=DRED if ci>=3 else INK,bold=(ci>=3),anchor=MSO_ANCHOR.MIDDLE,sp=1.0)
    y=add(y,Inches(0.62))
box(s,Inches(0.55),add(y,Inches(0.2)),Inches(11.9),Inches(1.85),fill=LRED)
txt(s,Inches(0.85),add(y,Inches(0.35)),Inches(11.3),Inches(1.6),
    '含意：①兵庫の70バンは本件と同じ「1998年式・ディーゼル・バン・業務車」で321万＝ランクルの競争力の証明（ただし走行4.9万km・全国公開の119入札）。\n②本件は北上・花巻・紫波限定の紙入札＝この全国競争が構造的に起きない。「全国なら100万超の車を地域の壁の内側で取る」のが本件の妙味。\n③ただし地元にも目利きはいる。「最低15万だから20万で取れる」という楽観はしない＝決定木A評価61.7〜67.8万の妥当性がこのデータで裏付けられた。',size=11.5,color=DRED,bold=True,sp=1.3)
footer(s)

# ============ 20c. 出口レイヤーと入札決定木 ============
s=prs.slides.add_slide(BLANK); bg(s)
header(s,'データ ｜ 2026-07-19追加','出口レイヤー分析 × 入札額の決定木（8/20の評価で自動決定）','出口がゼロになるシナリオがほぼ無い個体')
box(s,Inches(0.55),Inches(1.8),Inches(5.9),Inches(4.85),fill=BEIGE)
box(s,Inches(0.55),Inches(1.8),Inches(5.9),Inches(0.06),fill=RED)
txt(s,Inches(0.85),Inches(2.0),Inches(5.3),Inches(0.4),'出口レイヤー（ランクル100ディーゼル）',size=13.5,color=DRED,bold=True)
txt(s,Inches(0.85),Inches(2.5),Inches(5.4),Inches(4.0),
    '①整備済み小売：VX-D 〜350万／VXL-G-D 〜400万\n　（本件は直接届かない・天井の参考）\n②20万km超の買取実績：100万円以上あり\n③本件の実査定：100〜200万（旧車王・実測）★基準\n④現状渡しヤフオク直販：落札平均 約40.9万\n⑤公用車全般の官公庁落札：平均15.7万\n　（ランクルは前ページの通り大きく上に外れる）\n⑥廃車・部品取り：1HDエンジン価値で数十万\n\n→ 最悪ケース⑥でもゼロにならない＝上限さえ\n　守れば大負けしない構造',size=11.5,color=INK,sp=1.28)
box(s,Inches(6.75),Inches(1.8),Inches(5.95),Inches(4.85),fill=BEIGE)
box(s,Inches(6.75),Inches(1.8),Inches(5.95),Inches(0.06),fill=RED)
txt(s,Inches(7.05),Inches(2.0),Inches(5.4),Inches(0.4),'入札額の決定木（現物評価に連動）',size=13.5,color=DRED,bold=True)
dt=[('A 優良','下回り表面サビのみ・修復跡なし・漏れなし・ランプ消灯','617,000〜678,000円'),
    ('B 並','表面サビ広め・軽微減点','517,000円前後'),
    ('C 難あり','膨れ剥離・オイル漏れ・ランプ点灯','317,000円以下 or 見送り'),
    ('D 重症','貫通錆 or 修復跡(=修復歴あり扱い)','撤退')]
y=Inches(2.5)
for name,cond,price in dt:
    box(s,Inches(7.05),y,Inches(5.35),Inches(0.92),fill=WHITE)
    txt(s,Inches(7.2),add(y,Inches(0.06)),Inches(1.3),Inches(0.8),name,size=12,color=DRED,bold=True,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(8.55),add(y,Inches(0.06)),Inches(2.4),Inches(0.82),cond,size=9.5,color=INK,sp=1.1,anchor=MSO_ANCHOR.MIDDLE)
    txt(s,Inches(11.0),add(y,Inches(0.06)),Inches(1.35),Inches(0.8),price,size=10,color=INK,bold=True,anchor=MSO_ANCHOR.MIDDLE,sp=1.05)
    y=add(y,Inches(1.0))
txt(s,Inches(7.05),add(y,Inches(0.05)),Inches(5.4),Inches(0.35),'※掛け目0.5・諸費用12万・同額くじ回避の端数付き',size=9.5,color=GRAY)
footer(s)

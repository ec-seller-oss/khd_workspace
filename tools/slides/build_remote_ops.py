...
cols=[
 ...
 ("新Mac｜持ち出し・打席機",REDD,[
   ("● 商談・内見・現地の相棒",INK),
   ("● 基本は旧Macに“入って”使う",REDD),
   ("　（ゆーしと同じ・データは旧Mac側）",GRY),
   ("● 圏外/オフライン時だけローカル起動",INK),
   ("　→ 後で「保存して」で同期",GRY),
   ("● 打席直後に“その場で1行”投げる",INK),
   ("● スマホ1本でも旧Macに入れる",GRY),
 ]),
 ("ゆーしMac｜ゆーし主＋菊池も使える",GRY,[
   ("● 菊池のGoogle/Claudeログイン済",INK),
   ("　＋workspace clone済",GRY),
   ("● ＝菊池がすぐ使える3台目の実機",REDD),
   ("● 普段の主＝ゆーし（EC折半）",INK),
   ("● 制約は全台共通：開始pull/終了",INK),
   ("　push・書き手1台",GRY),
   ("● “触らない”でなく“同じ案件を",INK),
   ("　母艦と同時に書かない”",GRY),
 ]),
...
# ── S5b 接続モデル＆メモリ・負荷（追記）──
s=sl(); ft(s)
hdr(s,"CONNECTION & LOAD","接続モデルと、メモリ・負荷","入る側（クライアント）は軽い。重さは“入られる側”＝旧Macに集中する")
band(s,Inches(1.9),"新Mac・ゆーしMac・スマホは、全部「旧Mac」に入る（リモート）",
     "本体・データ・記憶は旧Macに1つだけ。だから書き手も実質1台＝同時編集の衝突が起きない。")
rows=[
 ("機器","役割","メモリ・負荷の目安"),
 ("旧Mac（8GB・M1 Air）","母艦＝全員が入る側","★重い側。全セッション＋リモート配信をここで背負う"),
 ("新Mac（ハイスペック）","打席機。基本は旧Macに入る／圏外時だけローカル","入るだけ＝超軽い(ブラウザ1枚)。ローカル起動時のみClaude分のRAM"),
 ("ゆーしMac","旧Macに入るだけ","超軽い(ブラウザ1枚)。本体メモリはほぼ食わない"),
 ("スマホ","旧Macに入るだけ","アプリ1つ。負荷ほぼゼロ"),
]
ry,rh=Inches(3.0),Inches(0.56)
cw=[Inches(3.2),Inches(4.6),Inches(4.43)]
for ri,row in enumerate(rows):
    cx=Inches(0.55); head=(ri==0); hot=(ri==1)
    for ci,cell in enumerate(row):
        bg=RED if head else (REDBG if hot else (CARD if ri%2 else GRYBG))
        bx(s,cx,ry+rh*ri,cw[ci],rh,bg,line=CARDLN,lw=0.8)
        tc=WHT if head else (REDD if hot and ci==0 else INK)
        t(s,cell,cx+Inches(0.16),ry+rh*ri,cw[ci]-Inches(0.28),rh,sz=11.5 if head else 11,
          bold=head or ci==0,col=tc,anchor=MSO_ANCHOR.MIDDLE,line_sp=1.0)
        cx=cx+cw[ci]
band(s,Inches(5.95),"⚠️ 旧Mac＝8GBが全体の上限。常時2セッション＋ゆーし1接続が現実的な天井。",
     "重くなったら、入る先を新ハイスペックMac3に切替＝母艦を格上げ（クライアント側は変えなくてOK）。")
t(s,"※実測(2026-06-28 旧Mac)：claude各セッション≒80〜140MB／空きメモリ33%／ロード平均 約2.0（8コア）。",
  Inches(0.6),Inches(6.96),Inches(12),Inches(0.3),sz=9,col=GRY)
...
for ri,row in enumerate(rows):
    ...
    for ci,cell in enumerate(row):
        ...
        cx=cx+cwidths[ci]
panel(s,Inches(0.55),Inches(4.78),Inches(12.23),Inches(2.05),"🔐 セキュリティ（人に勧める時の心得）",[
 ...
 ("⚠️ ゆーしMac＝第三者(ゆーし)の実機に菊池のGoogle/Claude/顧客・財務・家族データがclone＆ログイン済。フルアクセス共有を許容するか、用途限定(別アカウント)にするかは意識的に決める。",REDD),
],lh=0.46,bsz=11.5)
